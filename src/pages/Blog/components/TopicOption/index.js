                    ...
                    >
                        ...
                        <div style={{ display: "flex" }}>
                            <FormControl component="fieldset" className={classes.formControl}>
                                <FormLabel component="legend">Chọn Topic Bạn Muốn Trải Nghiệm</FormLabel>
                                <TextField id="standard-basic" label="Tên Bạn" onChange={(node) => onSetName(node.target.value)} value={name} />
                                <FormGroup>
                                    {allTopic.length > 0 && allTopic.map((name, index) => (
                                        <FormControlLabel
                                            control={<Checkbox checked={selectedTopics.includes(name)}
                                                onChange={(node) => onChosingTopic({ visible: node.target.checked, name: node.target.name })} name={name} />}
