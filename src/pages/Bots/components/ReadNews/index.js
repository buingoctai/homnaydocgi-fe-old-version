...
class ReadNews extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      url: 'https://viblo.asia/p/crawl-du-lieu-trang-web-voi-scrapy-E375zWr1KGW',
      crawlingStatus: null,
      data: null,
      taskID: null,
      uniqueID: null
  ...
  handleStartButton = (event) => {
    ...
    axios.post('http://localhost:8000/api/crawl/', { url: this.state.url })
      .then(resp => {
        this.setState({
          taskID: resp.task_id,
  ...
  checkCrawlStatus = () => {
    ...
    axios.get('http://localhost:8000/api/crawl/',
      { task_id: this.state.taskID, unique_id: this.state.uniqueID })
      .then(resp => {
        if (resp.data) {
  ...
  render() {
    // render componenet
    return (<div>
      <Button variant="contained" color="primary" onClick={() => this.handleStartButton()}>
        ĐỌC
       </Button>
    </div>)
  }
